...
class data_preprocess(object):
    ...
    def parse_file(self):
        data = pd.read_hdf(r'D:\BoYun Company\Https_Project\data\8.30\car\www.autohome.com.cn\train\8.30_car.h5', key='data')
        data.to_csv(r'D:\BoYun Company\Https_Project\data\8.30\car\www.autohome.com.cn\train\8.30_car.csv',index = False)

    # 制作一个lable-index的字典
    # 制作一个index-lable的字典
    def SSL_dic(self):
        ssl_data = pd.read_csv(r'D:\BoYun Company\Https_Project\data\8.30\car\www.autohome.com.cn\train\8.30_car.csv')
    ...
    def SSL_train_transform(self):
        ...
        dic = pickle.load(f)
        f.close()
        #ssl_data = pd.read_hdf(r'D:\BoYun Company\Https_Project\data\8.30\car\www.autohome.com.cn\train\8.30_car.h5', key='data')
        ssl_data = pd.read_csv(r'D:\BoYun Company\Https_Project\data\8.30\car\www.autohome.com.cn\train\8.30_car.csv')
        ssl_data.iloc[:,2000] = ssl_data.iloc[:,2000].fillna('null')  # 将ssl_data中url列所有空值赋值为'null'
        ssl_data = ssl_data[~ssl_data.iloc[:,2000].isin(['null'])]
        # URL为类标
    ...
    def SSL_test_transform(self):
        ...
        dic = pickle.load(f)
        f.close()
        ssl_data = pd.read_hdf(r'D:\BoYun Company\Https_Project\data\8.30\car\www.autohome.com.cn\test\8.30_car_test.h5', key='data')
        #ssl_data.to_csv(r'D:\BoYun Company\Https_Project\data\8.30\car\www.autohome.com.cn\test\8.30_car_test.csv', index=False)
        ssl_data.iloc[:, 2000] = ssl_data.iloc[:, 2000].fillna('null')  # 将ssl_data中url列所有空值赋值为'null'
        ssl_data = ssl_data[~ssl_data.iloc[:, 2000].isin(['null'])]
        # URL为类标
        url = np.unique(ssl_data.iloc[:, 2000].values.tolist()).tolist()
...
class QD_data(object):
    ...
    t：t为设定的测试url种类取值
    '''
    def __init__(self,w=10,t=96):
        self.w = w
        self.t = t
    def parse_file(self):
        data = pd.read_hdf(r'D:\BoYun Company\Https_Project\data\8.30\qidian\train\8.30_book{_train.h5', key='data')
        data.to_csv(r'D:\BoYun Company\Https_Project\data\8.30\qidian\train\8.30_book{_train.csv',index = False)

    # 制作一个lable-index的字典和index-lable的字典
    def QD_dic(self):
        qd_data = pd.read_csv(r'D:\BoYun Company\Https_Project\data\8.30\qidian\train\8.30_book{_train.csv')
        # URL为类标
        ...
        for i, k in enumerate(url):
            url_dict[k] = i
            index_lable[i] = k
        f = open(r'D:\BoYun Company\Https_Project\data\8.30\qidian\train\lable_index_dic.pickle', 'wb')
        f2 = open(r'D:\BoYun Company\Https_Project\data\8.30\qidian\train\index_lable_dic.pickle', 'wb')
        pickle.dump(url_dict, f)
        pickle.dump(index_lable, f2)
        f.close()

    def QD_train(self):
        f = open(r'D:\BoYun Company\Https_Project\data\8.30\qidian\train\lable_index_dic.pickle', 'rb')
        dic = pickle.load(f)
        f.close()
        qd_data = pd.read_csv(r'D:\BoYun Company\Https_Project\data\8.30\qidian\train\8.30_book{_train.csv')
        # URL为类标
    ...
    def QD_test_(self):
        ...
        dic = pickle.load(f)
        f.close()
        ssl_data = pd.read_hdf(r'D:\BoYun Company\Https_Project\data\8.30\qidian\test\8.30_book_test.h5', key='data')
        # URL为类标
        url = np.unique(ssl_data.iloc[:, 2000].values.tolist()).tolist()
        print('测试数据总共有{}种url'.format(len(url)))
        # 随机选取self.t种url
        slice_ = random.sample(url, self.t)
        #slice_ = ['https://www.qidian.com/free','https://www.qidian.com/all_pub']
        lables = {}
        for r in slice_:
            index = dic[r]
            lables[index] = r
