...
class SAX(object):
    ...
    def knn_predict(self,x_train,y_train,x_test,y_test,k=10):
        '''
        :param x_train:2-d array
        :param y_train:1-d array
        :param x_test:2-d array
        :param y_test:1-d array
        ...
        --------
        Gengrate topk nearest distance's y_predict about y_lable
        '''
        ...
        y_predict = []
        for loc1,x1 in enumerate(x_test):
            x1_dist = []
            compare_dist_ind = {}
            for loc2,x2 in enumerate(x_train):
                (x1String, x1Indices) = self.to_letter_rep(x1)
                (x2String, x2Indices) = self.to_letter_rep(x2)
                x1x2ComparisonDist = self.compare_strings(x1String, x2String)
                if x1x2ComparisonDist not in compare_dist_ind:
                    compare_dist_ind[x1x2ComparisonDist] = [loc2]
                else:
                    compare_dist_ind[x1x2ComparisonDist].append(loc2)
                x1_dist.append(x1x2ComparisonDist)
            topk = sorted(x1_dist)[:k]
            index = []
            for i in topk:
                index.extend(compare_dist_ind[i])
            topk_index = index[:k]
            topk_lable = [ytrain_dic[i] for i in topk_index]
            y_predict.append(topk_lable)
