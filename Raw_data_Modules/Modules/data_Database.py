...
class data_to_Data(object):
    ...
    def generate_url_path(self,datadir):
        '''
        ...
        :return: None
        '''
        # datadir = r"D:\BoYun Company\Https_Project\fpf_data"
        # urllst = r"D:\BoYun Company\Https_Project\fpf_data_temp\urllst.csv"
        self.fpf = FPFData(datadir)
        self.url_list_files = self.fpf._url_list_files
        self.url_path = 'url_path.csv'
        with open(self.url_path, "w") as u:
            for f in self.url_list_files:
        ...
        # writer.write("\n")
        with open(self.url_path) as reader:
            for line in reader:
                try:
                    ...
                    print(line)
                    break
                self.features = u.extract_url_features()
                self.g = flatten_features(self.features)
                self.g.extend([timestamp, url])

        self.conn = self.Database_conn()
        for r in self.g:
            # SQL 插入语句
            sql = "INSERT INTO visit_url_features_copy2 \
                   VALUES ()"
            try:
                # 执行sql语句
